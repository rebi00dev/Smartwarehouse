...
def action_build(node, DR, poses):
    return {'pick':lambda object: PickAction(node, dr=DR, poses=poses['Pick'], ingredient=object),
            ...
            'yolo':lambda : DetectAction(node, dr=DR),
...
def main():
    ...
    while True:
        try:
            ...
            # Yolo data 확인 (물건 list) =============================================
            target_object = actions_build['yolo']().execute()
            if not target_object:
                node.get_logger().info("감지된 물체가 없습니다. 재시도 중...")
                continue
            node.get_logger().info(f"타겟 감지: {target_object}")

            # pick ================================================================= 
            pick_success = actions_build['pick'](target_object).execute()
            if not pick_success:
                node.get_logger().error("물체를 잡는 데 실패했습니다. 처음으로 돌아갑니다.")
                continue
            node.get_logger().info(f"{target_object} pick 완료. home 위치 이동")

            # home =================================================================
            actions_build['home']().execute()
            node.get_logger().info(f"{target_object} 이동 완료. waypoint 이동 ")

            # waypoint =============================================================
            actions_build['waypoint']().execute()
            node.get_logger().info(f"{target_object} 이동 완료. place 시작")

            # place ================================================================
            actions_build['place'](target_object, dictonary).execute()
            node.get_logger().info(f"{target_object} place 완료. waypoint 이동.")

            # waypoint =============================================================
            actions_build['waypoint']().execute()
            node.get_logger().info(f"{target_object} 이동 완료. home 위치 이동")
